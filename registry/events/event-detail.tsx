/* oxlint-disable complexity, no-nested-ternary, unicorn/no-nested-ternary */
"use client";
...
const EventDetailView = ({ data, actions, appearance }: EventDetailProps) => {
  const resolved: NonNullable<EventDetailProps["data"]> = data ?? {
    event: demoEventDetails,
  };
            ...
            {showMap && event.venue_details.coordinates && (
              <div className="mt-4 aspect-video overflow-hidden rounded-lg bg-muted">
                <Suspense fallback={<MapPlaceholder />}>
                  <LazyLeafletMap
                    center={[
                      event.venue_details.coordinates.lat,
                      event.venue_details.coordinates.lng,
                    ]}
                    zoom={15}
            ...
            <div className="mt-3 rounded-lg border p-4">
                  ...
                  <div className="flex gap-4 text-xs text-muted-foreground">
                    {event.organizer.followers !== undefined && (
                      <span>
                        ...
                        <br />
                        <strong>
                          {formatNumber(event.organizer.followers)}
                        </strong>
                      </span>
                    )}
                    {event.organizer.eventsCount !== undefined && (
                      <span>
                        Events
                        <br />
                        <strong>{event.organizer.eventsCount}</strong>
                      </span>
                    )}
                    {event.organizer.hostingYears !== undefined && (
                      <span>
                        Hosting
                        <br />
                        <strong>{event.organizer.hostingYears} yrs</strong>
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className="mt-3 flex gap-2">
                <Button
                  ...
                  size="sm"
                  className="flex-1"
                  onClick={() => onContact?.(event.organizer)}
                >
                  Contact
                </Button>
                <Button
                  size="sm"
                  className="flex-1 bg-primary hover:bg-primary/90"
                  onClick={() => onFollow?.(event.organizer)}
                >
                  Follow
                </Button>
              </div>
              {event.organizer.trackRecord ||
                (event.organizer.responseRate && (
