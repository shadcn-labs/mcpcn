...
const TicketTierSelectView = ({
  data,
  actions,
  appearance,
  control,
}: TicketTierSelectProps) => {
  const resolved: NonNullable<TicketTierSelectProps["data"]> = data ?? {
  ...
  const updateQuantity = (tierIndex: number, delta: number) => {
    ...
    if (newQty === 0) {
      Reflect.deleteProperty(newSelections, tierIndex);
    }
    setSelections(newSelections);
  };

  const getSelectionsList = (
    sels: Record<number, number> = selections
  ): TicketSelection[] =>
    Object.entries(sels)
      .filter(([_, qty]) => qty > 0)
      .map(([indexStr, qty]) => {
        const tierIndex = Number.parseInt(indexStr, 10);
        const tier = tiers[tierIndex];
        return {
          fee: tier?.fee ?? 0,
          price: tier?.price,
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <Button
                        ...
                        onClick={() => updateQuantity(index, 1)}
                        disabled={
                          qty >= (tier.maxPerOrder ?? 10) ||
                          qty >= (tier.available ?? 100)
                        }
                      >
