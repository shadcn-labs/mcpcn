---
title: "Stat Card"
description: "Scrollable stat cards with values, trends, and change indicators."
base: base
component: true
---

<ComponentPreview name="stat-card" src="examples/stat-card-demo.tsx" />

## Installation

<CodeTabs>

<TabsList>
  <TabsTrigger value="cli">Command</TabsTrigger>
  <TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">

```bash
npx shadcn@latest add https://mcpcn.dev/r/stat-card.json
```

</TabsContent>

<TabsContent value="manual">

<Steps className="mb-0 pt-2">

<Step>Install the component dependencies:</Step>

```bash
npm install lucide-react
```

<Step>Copy and paste the component into your project.</Step>

<ComponentSource name="stat-card" title="components/ui/stat-card.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Usage

```tsx
import { StatCard } from "@/components/ui/stat-card";
```

```tsx
<StatCard />
```

## Composition

Use the following composition to build an `StatCard`:

```text
StatCard
└── StatCard.Content
```

## Examples

### With a custom heading

This example adds a custom heading before the component's `Content`. `Content` reads the root props from compound context, so the original Manifest UI view needs no repeated props.

<ComponentPreview
  name="stat-card"
  src="examples/stat-card-demo.tsx"
  variant="composed"
  showCode
/>

## Integrate with OpenUI

Register `StatCard` with `defineComponent`, describe its model-generated props with Zod, and include it in a library.

```tsx showLineNumbers
import { createLibrary, defineComponent } from "@openuidev/react-lang";
import { z } from "zod/v4";

import { StatCard } from "@/components/ui/stat-card";

const OpenUIStatCard = defineComponent({
  name: "StatCard",
  description:
    "Scrollable stat cards with values, trends, and change indicators.",
  props: z.object({}),
  component: () => <StatCard />,
});

export const stat_cardLibrary = createLibrary({
  root: "StatCard",
  components: [OpenUIStatCard],
});
```

For richer generated compositions, define the child primitives separately and connect them with `.ref` fields. See [Defining Components](https://www.openui.com/docs/openui-lang/defining-components) in the OpenUI documentation.

## API Reference

See the [Base UI](https://base-ui.com/react/overview/quick-start) documentation for more information.
